...
export const NoMembers = () => {
  ...
  return <div className={'rounded-md p-5 text-center'}>
   <span className={'w-full flex justify-center'}>
    ...
    <div className={'mt-4'}>
      <h2 className={'font-semibold text-xl'}>{t('noMembers.header')}</h2>
      <p className={'text-muted-foreground text-sm'}>{t('noMembers.description')}</p>
    </div>
    <div className={'mt-4'}>
      <Link to={LIST_LINKS.INVITES_NEW(listId)}
            className={cn(buttonVariants({ size: 'sm' }), 'gap-2')}>{t('noMembers.inviteMembers')}</Link>
    </div>
  </div>;
};